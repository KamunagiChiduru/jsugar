...
public final class Concepts{
	 ...
	 * @throws IllegalArgumentException if conceptInterface is null.
	...
	public static <T, R>R conceptCast(T obj, Class<R> conceptInterface){
		...
		if(!isInterface(conceptInterface.getModifiers())){
			throw new IllegalArgumentException(
				String.format("%s is not an interface type.", conceptInterface));
		}
		if(!isMetConcept(origin.getClass(), conceptInterface)){
			throw new ConceptUnmatchException(origin.getClass(), conceptInterface);
		}
		
		final ImmutableCollection<Method> concepts= ImmutableList.copyOf(conceptInterface.getMethods());
		@SuppressWarnings("unchecked")
		R casted= (R)Proxy.newProxyInstance( //
				...
				new InvocationHandler(){
					...
					public Object invoke(Object proxy, Method method, Object[] args) throws Throwable{
						if(concepts.contains(method)){
							method= origin.getClass().getMethod(method.getName(), method.getParameterTypes());
						}
